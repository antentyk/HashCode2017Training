...
def request_criteria(r):
    ...
    return (quantity * (endpoint.datacenterdelay) ** 0.1  * sum([item[1] for item in endpoint.servers]) ** 0.01) /\
           ((video.capacity ** (golden / 24)))
...
def take_video(endpointiid, videoiid):
    ...
    for item in endpoint.servers:
        ...
        if(video.iid in server.videos):return
        if(server.capacity < video.capacity):continue
...
for item in requestslst:
    counter += 1
    if(counter % 1000 == 0):
        percent = (counter * 100) / len(requestslst)
